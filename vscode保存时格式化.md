## vscode格式化
&emsp;&emsp;vscode支持通过clang-format工具对项目工程进行格式化，减少开发人员。Linux风格的格式化配置文件.clang-format内容如下:
```json
...
```
## 保存时格式化
&emsp;&emsp;vscode支持设置多种格式化时机，对于程序员，选择保存时格式化更合适。针对配置settings.json文件配置如下：
```json
{
