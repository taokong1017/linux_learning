...
## 设备树二进制文件
```C
qemu-system-aarch64 -machine virt,gic-version=2,dumpdtb=qemu-virt.dtb -cpu cortex-a53
```
&emsp;&emsp;通过qemu-system-aarch64输出设备树二进制，其中参数dumpdtb，指定输出的设备二进制文件名。

```C
dtc -O dtb -o qemu-virt.dtb  qemu-virt.dts
```
&emsp;&emsp;通过dtc工具将设备树配置文件转换为设备树二进制。

## 设备树配置文件
```C
dtc -O dts -o qemu-virt.dts  qemu-virt.dtb
```
&emsp;&emsp;通过dtc工具将设备树二进制转换为设备树配置文件。
...
/ {
	...
	chosen {
		...
		rng-seed = <0x17fe51b 0x8091e434 0x4c6da410 0xe6b1e644 0xa10d099b 0x1ba0b6bb 0xd51c978d 0x6f149567>;
		kaslr-seed = <0x41b9706d 0x10ae6e1a>;
	};
};
```
